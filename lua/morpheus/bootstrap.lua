...
local function load_module_file(module)
	...
	for _, config_path in ipairs(morpheus.supported_configs) do
...
function morpheus.user_opts(module, default, extend)
...
morpheus.updater = {
	options = morpheus.user_opts("updater", { remote = "origin", channel = "stable" }),
	snapshot = { module = "lazy_snapshot", path = vim.fn.stdpath("config") .. "/lua/lazy_snapshot.lua" },
	rollback_file = vim.fn.stdpath("cache") .. "/morpheus_rollback.lua",
}
local options = morpheus.updater.options
if morpheus.install.is_stable ~= nil then
	options.channel = morpheus.install.is_stable and "stable" or "nightly"
end
...
morpheus.user_terminals = {}
--- table of language servers to ignore the setup of, configured through lsp.skip_setup in the user configuration
morpheus.lsp = { skip_setup = morpheus.user_opts("lsp.skip_setup", {}), progress = {} }
--- the default colorscheme to apply on startup
morpheus.default_colorscheme = morpheus.user_opts("colorscheme", "astrotheme", false)
