return function(_, _)
  local lsp = require "morpheus.utils.lsp"
  ...
  vim.lsp.handlers["$/progress"] = function(_, msg, info)
    local progress, id = morpheus.lsp.progress, ("%s.%s"):format(info.client_id, msg.token)
  ...
  local setup_servers = function()
    vim.tbl_map(require("morpheus.utils.lsp").setup, morpheus.user_opts "lsp.servers")
    vim.api.nvim_exec_autocmds("FileType", { modeline = false })
    require("morpheus.utils").event "LspSetup"
  end
