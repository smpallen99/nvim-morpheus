...
--
-- This module can also loaded with `local updater = require("morpheus.utils.updater")`
--
-- @module morpheus.utils.updater
-- @see morpheus.utils
-- @copyright 2022
-- @license GNU General Public License v3.0

local git = require "morpheus.utils.git"
...
function M.generate_snapshot(write)
  local file
  local prev_snapshot = require(morpheus.updater.snapshot.module)
  ...
  if write == true then
    file = assert(io.open(morpheus.updater.snapshot.path, "w"))
    file:write "return {\n"
  end
...
function M.version(quiet)
  local version = morpheus.install.version or git.current_version(false) or "unknown"
  if morpheus.updater.options.channel ~= "stable" then version = ("nightly (%s)"):format(version) end
...
function M.update_packages()
  ...
  require("morpheus.utils.mason").update_all()
end
...
function M.create_rollback(write)
  ...
  if write == true then
    local file = assert(io.open(morpheus.updater.rollback_file, "w"))
...
function M.rollback()
  local rollback_avail, rollback_opts = pcall(dofile, morpheus.updater.rollback_file)
...
function M.update_available(opts)
  if not opts then opts = morpheus.updater.options end
  opts = require("morpheus.utils").extend_tbl({ remote = "origin" }, opts)
...
function M.update(opts)
  if not opts then opts = morpheus.updater.options end
  opts = require("morpheus.utils").extend_tbl(
  ...
  then
    ...
    if #breaking > 0 then
      echo {
        { "Cannot update to MorpheusNvim v4 due to changes in the installation and management\n", "ErrorMsg" },
    ...
    local summary = {
      { "MorpheusNvim updated successfully to ", "Title" },
      { git.current_version(), "String" },
      { "!\n", "Title" },
      {
        opts.auto_quit and "MorpheusNvim will now update plugins and quit.\n\n"
    ...
    -- if the user wants to auto quit, create an autocommand to quit MorpheusNvim on the update completing
    if opts.auto_quit then
      vim.api.nvim_create_autocmd("User", {
        desc = "Auto quit MorpheusNvim after update completes",
        pattern = "MorpheusUpdateComplete",
        command = "quitall",
      })
    end
