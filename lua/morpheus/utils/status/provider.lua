...
--
-- This module can be loaded with `local provider = require "morpheus.utils.status.provider"`
--
-- @module morpheus.utils.status.provider
-- @copyright 2023
-- @license GNU General Public License v3.0

local M = {}

local condition = require "morpheus.utils.status.condition"
local env = require "morpheus.utils.status.env"
local status_utils = require "morpheus.utils.status.utils"
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.fill }
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.signcolumn }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.numbercolumn }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.foldcolumn }
-- @see morpheus.utils.status.utils.stylize
function M.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require "morpheus.utils.ffi" -- get MorpheusNvim C extensions
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.tabnr() }
function M.tabnr()
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.spell() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.paste() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.macro_recording() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.showcmd() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.search_count() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.mode_text() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.percentage() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.scrollbar() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.close_button() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.filetype() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.filename() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.file_encoding() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.file_format() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.unique_path() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.file_modified() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.file_read_only() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.file_icon() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.git_branch() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.git_diff({ type = "added" }) }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.diagnostics({ severity = "ERROR" }) }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.lsp_progress() }
-- @see amorpheus.utils.status.utils.stylize
function M.lsp_progress(opts)
  local spinner = utils.get_spinner("LSPLoading", 1) or { "" }
  return function()
    local _, Lsp = next(morpheus.lsp.progress)
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.treesitter_status() }
-- @see morpheus.utils.status.utils.stylize
...
-- @usage local heirline_component = { provider = require("morpheus.utils.status").provider.str({ str = "Hello" }) }
-- @see morpheus.utils.status.utils.stylize
function M.str(opts)
