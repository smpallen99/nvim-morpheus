...
local morpheusevent = utils.event
...
autocmd({ "BufAdd", "BufEnter", "TabNewEntered" }, {
  ...
  callback = function(args)
    local buf_utils = require "morpheus.utils.buffer"
...
autocmd("BufDelete", {
  ...
  callback = function(args)
    ...
    vim.t.bufs = vim.tbl_filter(require("morpheus.utils.buffer").is_valid, vim.t.bufs)
    if removed then morpheusevent "BufsUpdated" end
...
autocmd("BufEnter", {
  desc = "Quit MorpheusNvim if more than one window is open and only sidebar windows are list",
...
autocmd("ColorScheme", {
  ...
  group = augroup("morpheus_highlights", { clear = true }),
  callback = function()
    if vim.g.colors_name then
      for _, module in ipairs { "init", vim.g.colors_name } do
        for group, spec in pairs(morpheus.user_opts("highlights." .. module)) do
          vim.api.nvim_set_hl(0, group, spec)
        end
      end
    end
    morpheusevent("ColorScheme", false)
  end,
})

autocmd({ "BufReadPost", "BufNewFile", "BufWritePost" }, {
  desc = "MorpheusNvim user events for file detection (MorpheusFile and MorpheusGitFile)",
...
cmd(
  "MorpheusChangelog",
  function() require("morpheus.utils.updater").changelog() end,
  { desc = "Check MorpheusNvim Changelog" }
)
cmd(
  "MorpheusUpdatePackages",
  function() require("morpheus.utils.updater").update_packages() end,
  { desc = "Update Plugins and Mason" }
)
cmd("MorpheusRollback", function() require("morpheus.utils.updater").rollback() end, { desc = "Rollback MorpheusNvim" })
cmd("MorpheusUpdate", function() require("morpheus.utils.updater").update() end, { desc = "Update MorpheusNvim" })
cmd("MorpheusVersion", function() require("morpheus.utils.updater").version() end, { desc = "Check MorpheusNvim Version" })
cmd("MorpheusReload", function() require("morpheus.utils").reload() end, { desc = "Reload MorpheusNvim (Experimental)" })
